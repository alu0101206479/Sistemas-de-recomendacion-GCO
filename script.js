...
function correlacion_pearson(matriz, medias, usuario1, usuario2) {
  let vec_aux = []
      ...
      if (i !== j) {
        ...
        switch (metrica) {
          case "Correlación de Pearson":
            ...
            if (indice === -1) {
              similiaridades.push([i, [[j, correlacion_pearson(matriz, medias_usuarios, i, j)]]]); // Se introduce por primera vez el usuario y su similitud con otro usuario
            } else {
              similiaridades[indice][1].push([j, correlacion_pearson(matriz, medias_usuarios, i, j)]) // Se introduce la similitud con otro usuario más
            }
...
function tratar_datos() {
  if (matriz_entrada.length == 0) { // Comprobamos si la matriz se ha recogido del fichero correctamente
    ...
    if (numero_vecinos > matriz_entrada.length-1) { // Comprobamos si el número de vecinos escrito es posible
      ...
      let salida_syr = sistema_recomendacion(matriz_entrada, metrica, numero_vecinos, tipo_prediccion) // Realizamos la función del sistema de recomendación
      ...
      for (let i = 0; i < salida_syr[1].length; i++) {
        salida = salida + "· Usuario " + (parseInt(salida_syr[1][i][0])+1) + ":"
        for (let j = 0; j < salida_syr[1][i][1].length; j++) {
          salida = salida + "<br>Usuario " + (parseInt(salida_syr[1][i][0])+1) + " - Usuario " + (parseInt(salida_syr[1][i][1][j][0])+1) + ": <b>" + salida_syr[1][i][1][j][1] + "</b>"
        }
        salida = salida + "<br>"
      }
